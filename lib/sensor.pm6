...
my $screencapture-file = "screencapture.jpg";

sub take-screencapture-as-mat() {
    take-screencapture();
    my $mat = imread($screencapture-file);
    unlink $screencapture-file;
    return $mat;
}

sub take-screencapture() {
    if $*DISTRO.name eq "macosx" {
        take-screencapture-on-mac;
    } else {
        die "take-screencapture not implemented on $*DISTRO";
    }
}

sub take-screencapture-on-mac() {
    # my $start = DateTime.new(now);
    shell "screencapture -t jpg -x -o $screencapture-file";
    # my $end = DateTime.new(now);
    # say "screen capture time cost: {$end - $start}s";
}
